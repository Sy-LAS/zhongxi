...
namespace sensor_interfaces
{
    ...
    private:
        ...
        int failure_count_;
        static const int MAX_FAILURE_COUNT = 5;  // 最大失败次数，超过此值将重启连接
        
        void reset_lidar_connection();
    };
}
...
namespace sensor_interfaces
{
    LidarDriver::LidarDriver()
        : SensorNode("lidar_driver"), laser(nullptr), scan_pub_(nullptr), 
          is_initialized_(false), failure_count_(0)
    {
        // 初始化参数 - 参考YDLidar官方驱动实现
        // 获取参数
        ...
        this->declare_parameter<int>("baudrate", 115200);  // X3 Pro使用115200波特率
        this->declare_parameter<int>("lidar_type", TYPE_TRIANGLE);  // 三角测距雷达
        this->declare_parameter<int>("device_type", YDLIDAR_TYPE_SERIAL);
        this->declare_parameter<int>("sample_rate", 3);  // X3 Pro使用3k采样率
        ...
        this->declare_parameter<bool>("isSingleChannel", true);  // X3 Pro是单通道
        this->declare_parameter<bool>("intensity", false);
        this->declare_parameter<bool>("support_motor_dtr", false);  // X3 Pro不需要DTR控制
        this->declare_parameter<double>("angle_max", 180.0);  // 角度单位为度
        this->declare_parameter<double>("angle_min", -180.0);  // 角度单位为度
        this->declare_parameter<double>("range_max", 8.0);  // X3 Pro最大测距
        this->declare_parameter<double>("range_min", 0.12);  // X3 Pro最小测距
        this->declare_parameter<double>("frequency", 5.0);  // 降低频率以提高稳定性
        this->declare_parameter<bool>("invalid_range_is_inf", true);
        ...
        if (initialize_sensor()) {
            ...
            failure_count_ = 0;
            RCLCPP_INFO(this->get_logger(), "YDLidar X3 Pro设备初始化成功");
        } else {
            RCLCPP_ERROR(this->get_logger(), "YDLidar X3 Pro设备初始化失败");
        }
    ...
    bool LidarDriver::initialize_sensor()
    {
        ...
        /// one-way communication - single channel
        ...
        /// Motor DTR - doesn't need DTR control for X3 Pro
        ...
        if (ret) {
            ...
            if (!ret) {
                RCLCPP_ERROR(this->get_logger(), "启动YDLidar X3 Pro设备扫描失败: %s", laser->DescribeError());
                ...
                if (!ret) {
                    RCLCPP_ERROR(this->get_logger(), "第二次尝试启动YDLidar X3 Pro设备扫描也失败: %s", laser->DescribeError());
                } else {
                    RCLCPP_INFO(this->get_logger(), "第二次尝试启动成功");
                }
            } else {
                RCLCPP_INFO(this->get_logger(), "YDLidar X3 Pro设备扫描已启动");
            }
        } else {
            RCLCPP_ERROR(this->get_logger(), "YDLidar X3 Pro设备初始化失败: %s", laser->DescribeError());
        }
        ...
        if (!ret) {
            ...
            RCLCPP_ERROR(this->get_logger(), "4. 确保使用正确的参数配置（特别是isSingleChannel=true）");
    ...
    void LidarDriver::publish_scan()
    {
        ...
        if (laser && is_initialized_) {
            ...
            if (laser->doProcessSimple(scan)) {
                if (scan.points.empty()) {
                    ...
                    if (failure_count_ > MAX_FAILURE_COUNT) {
                        reset_lidar_connection();
                    }
                    return;
                }
                
                failure_count_ = 0; // 重置失败计数
                ...
                // 输出调试信息，显示获取到的数据点数
                RCLCPP_DEBUG(this->get_logger(), "成功发布激光扫描数据，点数: %zu", scan.points.size());
            }
            else
            {
        ...
        // 如果传感器未初始化成功，则不发布任何数据
            ...
            RCLCPP_WARN_ONCE(this->get_logger(), "YDLidar X3 Pro设备未初始化，暂停发布scan数据");
            ...
            if (!is_initialized_) {
                if (initialize_sensor()) {
                    is_initialized_ = true;
                    failure_count_ = 0;
                    RCLCPP_INFO(this->get_logger(), "YDLidar X3 Pro重新初始化成功");
                }
            }
        }
    }
